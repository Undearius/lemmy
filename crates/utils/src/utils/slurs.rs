...
mod test {
  ...
  fn test_slur_filter() -> LemmyResult<()> {
    let slur_regex = RegexBuilder::new(r"(fag(g|got|tard)?\b|cock\s?sucker(s|ing)?|ni[gq]{2,}[e3]?r[sz]?|mudslime?s?|kikes?|\bspi[ck]s?\b|\bchinks?|gooks?|bitch(es|ing|y)?|whor(es?|ing)|\btr[a@]nn?(y|ies?)|\b(b|re|r)tard(ed)?s?)").case_insensitive(true).build()?;
    let test =
